...
export default function ConsignmentDeliveryPage() {
  ...
  const onScanEnter = (e) => {
    ...
    if (p) {
      ...
      if (i>=0) {
        ...
        setLines([...lines, { productId: p.id, qty: 1 }]);
      }
      setScan('');
    } else {
  ...
  const data = lines.map(l => {
    ...
    const shop = consignmentShops.find(s=> s.id===shopId);
    const unitNet = calcNetUnitPrice({ product: p, category: cat, shop, categoryOverride: ov });
    const qty = Number(l.qty)||0;
    const amountNet = unitNet * qty;
    const pct = resolveCommissionPct({ categoryOverride: ov, shop });
    const unitGross = Number(p.salePrice || p.basePrice || 0);
  ...
  const canSave = shopId && lines.length>0 && lines.every(l => Number(l.qty)>0);
  ...
  const onSave = () => {
    ...
    const payload = {
      ...
      date,
      shopId,
      lines: lines.map(l => ({ productId: l.productId, qty: Number(l.qty)||0 })),
      totals
    };
    if (typeof addConsignmentDelivery === 'function') addConsignmentDelivery(payload);
    alert('บันทึกใบส่งฝากขาย (mock)');
    setLines([]);
  };
