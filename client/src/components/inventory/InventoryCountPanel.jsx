// client/src/components/inventory/InventoryCountPanel.jsx
...
export default function InventoryCountPanel({ branches = [], partners = [] }) {
  ...
  async function finalize(){
    ...
    try {
      ...
      setSession(res.data); // backend จะคืน session ที่ปิดแล้ว + สรุป delta
    } finally { setSaving(false); }
  }
  ...
  return (
    <Card className="p-5 bg-gradient-to-b from-[#9db9ff] to-[#6f86ff] text-white shadow-md">
      ...
      <div className="rounded-2xl bg-white/95 p-3 mt-4 text-slate-800 overflow-hidden">
