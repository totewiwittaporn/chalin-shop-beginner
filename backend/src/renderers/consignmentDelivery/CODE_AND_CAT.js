// backend/src/renderers/consignmentDelivery/CODE_AND_CAT.js
// Render แบบ "ใช้รหัสและหมวดร้านฝากขาย"

export function renderCODE_AND_CAT(doc) {
  const money = (v) =>
  ...
  for (const l of lines) {
    const code = l.partnerCategoryCode || "-";
    const name = l.partnerCategoryName || "ไม่ระบุ";
    const key = code + "||" + name;
  ...
  <div class="head">
    <div>
      <div class="title">ใบส่งสินค้า (ร้านฝากขาย) - รหัสและหมวด</div>
  ...
  for (const [key, rows] of groups.entries()) {
    ...
    let groupSum = 0;
    html += `<div class="group"><div><strong>รหัสหมวด:</strong> ${code} — ${name}</div><table>
      <thead><tr>
